...
entry_test(EntryA, InitialA, EntryB, InitialB) ->
	...
	EntryB ! {read, 2, self()},
	T2 = recv(),
	?assertMatch({2, _, InitialB, _}, T2), %% This "Time" should be equal to the previous one
	...
	EntryB ! {read, 4, self()},
	?assertMatch({4, _, InitialB, Time2}, recv()), %% This "Time" should be equal to the previous one.

	EntryA ! {check, 5, Time1, self()}, %% Time1 should not check, since EntryA has been written
	?assertMatch({5, abort}, recv()),
	EntryA ! {check, 6, Time3, self()}, %% But Time3 should check
	?assertMatch({6, ok}, recv()),

	%% EntryB should go independently from all this
	EntryB ! {read, 7, self()},
	?assertMatch({7, _, InitialB, Time2}, recv()), %% This "Time" should be equal to the previous one.
...
handler_test() ->
	...
	Self = self(),
	?assertMatch({validate, 4,
		[{Entry1, _}],
		[{2, Entry2, 200}, {3, Entry3, 300}], Self}, recv()),
